...
gui 개발에 사용될 라이브러리를 선택해야 하는데, 그대로 파이썬으로 개발할 생각이라면 사실상 `PyQT`정도가 있겠다. 그러나 저 프레임워크에 그다지 좋은 기억은 없기 때문에, 아예 다른 언어를 사용해보기로 했다.
...
뭐 그건 그렇고, 본인은 잘 사용하기만 하면 되는 입장이라 C++과의 차이점을 대충대충 훑어보고 첫째 날이 마무리되었다.
...
MVVM이란 전체 로직을 **Model, View, VideModel**의 세 단위로 철저히 분리해서 개발하는 방식으로 보인다. 각각 데이터 구조, 유저 인터페이스, 뷰와 모델 간의 연결장치 정도로 보인다. 뷰모델에 실제로 핵심적인 로직이 들어가는 것 같다.
...
**MVVM** 패턴에서 함수를 바인딩할 때는 `ICommand` 인터페이스를 상속한 클래스를 참조하게 된다. 중요한 건 컨트롤의 `command` 프로퍼티에 이 객체가 바인딩되는데, **해당 프로퍼티를 가진 객체가 기본적으로 Button밖에 없다는 사실.**
